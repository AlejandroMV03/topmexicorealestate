...
function App() {
  ...
  return (
    <>
      {/* Header fijo */}
      <header className="fixed top-0 left-0 w-full bg-red-700 text-white p-4 shadow-lg flex justify-center items-center z-50" style={{ height: "4rem" }}>
        <div className="flex items-center gap-3">
          <img src="/top.png" alt="Top Mexico Logo" className="h-10 w-auto object-contain" />
          <h1 className="text-2xl md:text-3xl font-bold">TopMexico Real Estate</h1>
        </div>
      </header>

      {/* Contenedor principal que usa flex para ocupar toda la pantalla */}
      <main
        className="flex flex-col"
        style={{ height: "100vh", paddingTop: "4rem", paddingBottom: "7rem" }} // same as header + footer height
      >
        {/* Contenedor con scroll que ocupa todo el espacio disponible */}
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-6 flex-1 min-w-[200px]">
          <img src="/top.png" alt="Top Mexico Logo" className="h-8 w-auto object-contain" />
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 flex-1 min-w-[160px] text-center md:text-left">
          ...
          <button
            onClick={() =>
              window.open("https://www.topmexicorealestate.com/LP-contactus.php", "_blank")
            }
